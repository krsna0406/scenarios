...
print(" COLUMNS HAVOING NULLS  1111")
df1.select([sum( col(clm).isNull().cast(IntegerType()) ).alias(clm) for clm in df1.columns ]).show()

# trying by declaring list and calling list as param in select
print(" COLUMNS HAVOING NULLS 22222")
collst= [sum( col(column).isNull().cast(IntegerType()) ).alias(column) for column in df1.columns ]

df1.select(*collst).show()

print("COLUMN RENAMING---")

renlst= [ col(column).alias(str(column)+'_ ' )   for column in df1.columns]

df1.select(*renlst).show()
